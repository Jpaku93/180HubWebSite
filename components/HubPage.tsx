...
const HubPage: React.FC<HubPageProps> = ({ onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-brand-black text-white animate-fadeIn">
      ...
      {/* Past Events Section */}
      <section className="py-20 px-8 bg-brand-charcoal border-t border-white/5 relative overflow-hidden">
         {/* Decorative elements */}
         <div className="absolute top-0 right-0 w-96 h-96 bg-brand-purple/5 blur-[100px] rounded-full pointer-events-none" />

         <div className="max-w-7xl mx-auto relative z-10">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 border-b border-white/10 pb-4">
              <div>
                <span className="text-brand-purple font-mono text-sm tracking-widest">THE ARCHIVE</span>
                <h2 className="text-4xl md:text-6xl font-display font-bold text-white uppercase mt-2">
                  Past Events
                </h2>
              </div>
              <p className="text-gray-500 text-sm md:text-right mt-4 md:mt-0 max-w-xs">
                A look back at the nights that shaped us.
              </p>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {pastEvents.map((event) => (
                <div key={event.id} className="group relative aspect-[3/4] bg-brand-black overflow-hidden rounded-sm cursor-pointer shadow-lg hover:shadow-brand-purple/20 transition-all duration-500">
                  <img 
                    src={event.image} 
                    alt={event.title} 
                    className="absolute inset-0 w-full h-full object-cover grayscale group-hover:grayscale-0 transition-transform duration-700 group-hover:scale-110" 
                  />
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-6 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-300">
                    <span className="inline-block bg-brand-lime text-black text-[10px] font-bold px-2 py-0.5 mb-2 transform -skew-x-12 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">
                      {event.date}
                    </span>
                    <h3 className="text-xl md:text-2xl font-display font-bold text-white uppercase leading-none group-hover:text-brand-lime transition-colors">
                      {event.title}
                    </h3>
                  </div>
                </div>
